...
public class lottoChoice extends JFrame {
 ...
 ArrayList<Integer> UserGetLottoNumbers = new ArrayList<Integer>(choiceLottoNumbers); // 유저가 선택 or 자동으로 선택한 숫자들 리스트
 JPanel pnl;
 JPanel selectPnl;
 ...
 public lottoChoice() {
    ...
    for (int i = 0; i < userNumberImages.length; i++) {
       userNumberImages[i] = new JLabel("번호 이미지" + (i));
    ...
    for (int i = 0; i < lblAlls.length; i++) {
       lblAlls[i] = new JLabel((ImageIcon) numberToImage.numberImage.get(i + 1)); // 0부터 44까지 되기때문에 +1을 했다.
       pnl.add(lblAlls[i]);
          ...
          public void mousePressed(MouseEvent e) {
             ...
             if (choiceLottoNumbers.size() < 7 && UserGetLottoNumbers.size() < 7) { // 6개 다찼을때는 조건문이 바로 나가져서
                ...
                for (int i = 0; i < lblAlls.length; i++) {
                   ...
                   if (clickLabel.equals(lblAlls[i])) { // 클릭 이벤트가 발생한 객체의 source와 label이미지객체의 source가 같으면 click
                ...
                if (clickcount % 2 != 0 && choiceLottoNumbers.size() < 6) {
                   lblAlls[clickNumber - 1]
                         .setIcon((ImageIcon) numberToImage.numberImage.get(clickNumber + 45)); // 수정 할꺼!!!!
                   ...
                   lblAlls[clickNumber - 1].setIcon((ImageIcon) numberToImage.numberImage.get((clickNumber)));
                } else {
       ...
       public void actionPerformed(ActionEvent e) {
          ...
          while (UserGetLottoNumbers.size() < 6) {
             ...
             UserGetLottoNumbers = new ArrayList<Integer>(choiceLottoNumbers); // 랜덤 숫자 이미지 삽입 이랑 순서 바뀌어서 중복숫자가
                                                                // 뽑혔다...
             lblAlls[random - 1].setIcon((ImageIcon) numberToImage.numberImage.get((random) + 45));
             Collections.sort(UserGetLottoNumbers);
          }
...
//          }
    ...
    checkButton.addActionListener(new ActionListener() {
       ...
       public void actionPerformed(ActionEvent e) {
          if (UserGetLottoNumbers.size() == 6) {
             for (int i = 0; i < 6; i++) {
                userNumberImages[i]
                      .setIcon((ImageIcon) numberToImage.numberImage.get(UserGetLottoNumbers.get(i))); // URL에서
                                                                                     // 숫자
                                                                                     // 0
                                                                                     // 이
                                                                                     // 이미지
    ...
    /////////////////////////////////////////// Reset버튼 만들기 시작
    /////////////////////////////////////////// ////////////////////////////////////////////////////
    resetButton.addActionListener(new ActionListener() { // *오류가 났다 RESET을 하면 자동으로 받은 번호는 지워지지만 유저가 선택한 번호는 지워지지
                                              // 않는다. **이거 오늘 꼭 해결한다
                                              // 의심가는거 1번 UserGetLottoNumbers의 remove에서 값을 잘못 넣은것 (x)
                                              // choiceLottoNumbers.clear(); -> List모든 값을 없애는 메소드를 썼지만
                                              // 오류가 났다.
                                              // 2번 69번째 줄에서 new를 만들었기 때문에 참조가 바뀌었나?
                                              // 확실한건 Random으로 받아 넣은 List참조값과 클릭해서 즉 Select으로 받아 넣은
                                              // List참조값
                                              // 3번 논리 자체가 잘못되어서 갈아야 하나...? 제발.... 아니길
                                              // *****해결***** SET값도 없애야지만 초기화가 됐다.
                                              // RESET버튼을 눌리면 List값은 다 지워지지만 Set값은 남아있다. 그상태에서 숫자를
                                              // 선택하면 지워지지않은 Set의 숫자와 클릭해서 생긴 Set숫자가 다 List안에 추가되게 된다.
       @Override
       public void actionPerformed(ActionEvent e) {
          if (UserGetLottoNumbers.size() > 0) {
             ...
             for (int i = 0; i < UserGetLottoNumbers.size(); i++) { // clear하고 난 뒤 이미지를 지우면 데이터 값이 없으므로 index가 없어
                                                       // index 오류가 나타난다.
                lblAlls[UserGetLottoNumbers.get(i) - 1]
                      .setIcon((ImageIcon) numberToImage.numberImage.get(UserGetLottoNumbers.get(i)));
             ...
             // lblAll[UserGetLottoNumbers.get(i) - 1].setIcon((ImageIcon)
             // numberToImage.numberImage.get(UserGetLottoNumbers.get(i)));
          } else {
    ...
    /////////////////////////////////////////// Reset버튼 만들기 마지막
    /////////////////////////////////////////// ////////////////////////////////////////////////////
    JButton MPbtn = new JButton("연습버튼");
    pnl.add(MPbtn);
    MPbtn.addActionListener(new ActionListener() {
        ...
        public void actionPerformed(ActionEvent e) {
           if (UserGetLottoNumbers.size() == 6) {
              for (int i = 0; i < 6; i++) {
                 mypage.chnumlist.get(i).setIcon((ImageIcon)numberToImage.numberImage.get(UserGetLottoNumbers.get(i)));
              }
